class Author < ApplicationRecord
    ...
    before_create do 
        ...
        if self.name.downcase =~ /(filho|filha|neto|neta|sobrinho|sobrinha|junior)/ 
            ...
            name_formated = self.name.split(/(\sda\s|\sde\s|\sdas\s|\sdos\s)/i)
            self.name_formated =  "#{name_formated.last.upcase}, #{name_formated.first} #{name_formated.second.downcase}"
        else
            name_formated = self.name.split(' ')
